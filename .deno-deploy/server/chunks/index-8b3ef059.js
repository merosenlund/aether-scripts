import { m as mount, u as unmount } from './index-server-db57e4a7.js';
import { e as ensure_array_like, b as attr, a as attr_class, s as stringify, c as escape_html, i as spread_props, l as derived } from './dev-db1ab9cf.js';
import { I as Icon } from './Icon-f47d171f.js';
import { c as contextEngine, S as Sparkles, a as reduceWikiEvents } from './contextEngine.svelte-fa9b4d62.js';
...
//#region node_modules/.deno/@lucide+svelte@1.16.0/node_modules/@lucide/svelte/dist/icons/circle-question-mark.svelte
...
//#endregion
//#region src/lib/editor/extensions/GMNote.ts
var GMNote = Node3.create({
	name: "gmNote",
	group: "block",
	content: "inline*",
	parseHTML() {
...
//#endregion
//#region src/lib/editor/extensions/DiceRoller.ts
var DiceRoller = Node3.create({
	name: "diceRoller",
	group: "inline",
	inline: true,
	atom: true,
	...
	renderHTML({ HTMLAttributes }) {
		const text = `🎲 ${HTMLAttributes.formula || "1d20"} = ${HTMLAttributes.result !== null ? HTMLAttributes.result : "?"}`;
		return [
			"span",
			mergeAttributes(HTMLAttributes, {
				"data-type": "dice-roller",
				class: "inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-200 cursor-pointer select-none mx-1"
			}),
			text
		];
	},
	renderText({ node }) {
		return `🎲 ${node.attrs.formula || "1d20"} = ${node.attrs.result !== null && node.attrs.result !== void 0 ? node.attrs.result : "?"}`;
	}
});
//#endregion
//#region src/lib/editor/extensions/StatBlock.ts
var StatBlock = Node3.create({
	name: "statBlock",
	group: "block",
	content: "block*",
	parseHTML() {
	...
	renderHTML({ HTMLAttributes }) {
		return [
			...
			mergeAttributes(HTMLAttributes, {
				...
				class: "bg-stone-50 dark:bg-stone-900 border border-stone-300 dark:border-stone-700 p-4 my-4 rounded-lg shadow-sm font-serif"
			}),
			0
		];
	}
...
//#endregion
//#region src/lib/editor/extensions/OddsCheck.ts
var OddsCheck = Node3.create({
	name: "oddsCheck",
	group: "inline",
	inline: true,
	atom: true,
	...
	renderHTML({ HTMLAttributes }) {
		...
		return [
			...
			mergeAttributes(HTMLAttributes, {
				...
				class: `inline-flex items-center px-2 py-0.5 rounded text-xs font-bold shadow-sm select-none mx-1 cursor-pointer transition-all ${roll === "?" ? "bg-stone-800 text-stone-400" : isSuccess ? "bg-emerald-500/20 text-emerald-400 border border-emerald-500/30" : "bg-rose-500/20 text-rose-400 border border-rose-500/30"}`
			}),
			text
		];
	},
	renderText({ node }) {
		...
		const roll = node.attrs.roll !== null && node.attrs.roll !== void 0 ? node.attrs.roll : "?";
		const resultText = roll !== "?" && Number(roll) <= Number(target) ? "YES" : "NO";
		const exceptionalThreshold = Math.floor(target / 5);
		const failureThreshold = 100 - Math.floor((100 - target) / 5);
		let label = resultText;
		if (roll !== "?") {
...
//#endregion
//#region src/lib/components/mechanics/Clock.svelte
function Clock($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		let { entityId = null, blockId = null, segments = 4, filled = 0, name = "", action = "create" } = $$props;
		const derivedState = derived(() => {
			...
			if (blockId) {
				...
				if (index !== -1) slicedEvents = contextEngine.rawEvents.slice(0, index + 1);
				else {
					...
					if (blockIndex !== -1) {
						...
						slicedEvents = contextEngine.rawEvents.filter((e) => !e.block_id || visibleBlocks.has(e.block_id));
					}
				}
			}
			return reduceWikiEvents(slicedEvents, null, contextEngine.baseEntities).get(entityId) || null;
		});
		const finalSegments = derived(() => derivedState()?.metadata?.segments !== void 0 ? derivedState().metadata.segments : segments);
		const finalFilled = derived(() => derivedState()?.metadata?.filled !== void 0 ? derivedState().metadata.filled : filled);
		const finalName = derived(() => derivedState()?.name || name);
		const validSegments = derived(() => [
			4,
			6,
			8,
			10,
			12
		].includes(finalSegments()) ? finalSegments() : 4);
		const blockEvent = derived(() => blockId ? contextEngine.rawEvents.find((e) => e.block_id === blockId) : null);
		const textElement = derived(() => blockEvent()?.payload?.reason || blockEvent()?.payload?.description || blockEvent()?.payload?.content);
		function getSegmentPath(index, total) {
			...
			return `M ${center} ${center} L ${startX} ${startY} A ${radius} ${radius} 0 ${endAngle - startAngle <= 180 ? 0 : 1} 1 ${endX} ${endY} Z`;
		}
		$$renderer.push(`<div data-component="clock-block-wrapper" class="group relative flex items-center gap-4 p-1"><div data-component="clock-inner-column" class="relative flex flex-col items-center gap-2">`);
		if (action === "increment") {
			$$renderer.push("<!--[0-->");
			$$renderer.push(`<div data-component="action-badge-increment" class="absolute top-0 right-0 z-10 animate-pulse rounded-full border border-emerald-400 bg-emerald-500 px-1.5 py-0.5 font-sans text-[9px] font-black text-stone-950 shadow-lg select-none">+1</div>`);
		} else if (action === "decrement") {
			$$renderer.push("<!--[1-->");
			$$renderer.push(`<div data-component="action-badge-decrement" class="absolute top-0 right-0 z-10 animate-pulse rounded-full border border-rose-400 bg-rose-500 px-1.5 py-0.5 font-sans text-[9px] font-black text-stone-950 shadow-lg select-none">-1</div>`);
		} else $$renderer.push("<!--[-1-->");
		$$renderer.push(`<!--]--> <svg viewBox="0 0 100 100" class="h-16 w-16 transform transition-all duration-300 group-hover:scale-105 group-hover:drop-shadow-[0_0_8px_rgba(245,158,11,0.2)]"><circle cx="50" cy="50" r="45" class="fill-stone-950 stroke-stone-800 stroke-[1.5]"></circle><!--[-->`);
		const each_array = ensure_array_like(Array(validSegments()));
		for (let i = 0, $$length = each_array.length; i < $$length; i++) {
			each_array[i];
			$$renderer.push(`<path${attr("d", getSegmentPath(i, validSegments()))}${attr_class(`stroke-stone-950 stroke-[1.5] transition-all duration-500 ${stringify(i < finalFilled() ? "fill-primary drop-shadow-[0_0_4px_var(--color-primary)]" : "fill-stone-900/60")}`)}></path>`);
		}
		$$renderer.push(`<!--]--></svg> `);
		if (finalName()) {
			$$renderer.push("<!--[0-->");
			$$renderer.push(`<div data-component="clock-labels" class="flex flex-col items-center"><span data-component="clock-name" class="text-center text-xs leading-tight font-bold text-stone-300 transition-colors group-hover:text-white">${escape_html(finalName())}</span> <span data-component="clock-action" class="mt-1 text-[9px] font-bold tracking-widest text-stone-500 uppercase">`);
			if (action === "create") {
				$$renderer.push("<!--[0-->");
				$$renderer.push(`Start (${escape_html(finalFilled())}/${escape_html(validSegments())})`);
			} else if (action === "increment") {
				$$renderer.push("<!--[1-->");
				$$renderer.push(`Incremented (${escape_html(finalFilled())}/${escape_html(validSegments())})`);
			} else if (action === "decrement") {
				$$renderer.push("<!--[2-->");
				$$renderer.push(`Decremented (${escape_html(finalFilled())}/${escape_html(validSegments())})`);
			} else {
				$$renderer.push("<!--[-1-->");
				$$renderer.push(`${escape_html(action)} (${escape_html(finalFilled())}/${escape_html(validSegments())})`);
			}
		...
		if (textElement()) {
			$$renderer.push("<!--[0-->");
			$$renderer.push(`<div data-component="clock-reason-container" class="flex flex-col justify-center border-l border-white/10 pl-4 max-w-[200px] shrink-0"><span data-component="clock-reason-label" class="text-[9px] font-bold tracking-widest text-stone-500 uppercase">${escape_html(blockEvent()?.payload?.reason ? "Reason" : "Details")}</span> <p data-component="clock-reason-text" class="mt-1 text-xs italic text-stone-400 leading-normal">"${escape_html(textElement())}"</p></div>`);
		} else $$renderer.push("<!--[-1-->");
		$$renderer.push(`<!--]--></div>`);
	});
}
//#endregion
//#region src/lib/editor/extensions/ClockBlock.ts
var ClockBlock = Node3.create({
	name: "clockBlock",
	group: "block",
	atom: true,
	addAttributes() {
	...
	parseHTML() {
		return [{ tag: "div[data-type=\"clockBlock\"]" }];
	},
	renderHTML({ HTMLAttributes }) {
	...
	renderText({ node }) {
		...
		return `[CLOCK: ${name} - ${actionLabel} (${filled}/${segments})${reason ? ` | "${reason}"` : ""}]`;
	},
	addNodeView() {
		return ({ node, editor, getPos }) => {
			...
			mount(Clock, {
				...
				props: {
					entityId,
					blockId,
					segments: node.attrs.segments || 4,
...
//#endregion
//#region src/lib/components/mechanics/ProgressTrack.svelte
function ProgressTrack($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		let { entityId = null, blockId = null, max = 10, current = 0, name = "" } = $$props;
		const activeBlockIdsSetForNode = derived(() => {
			if (!blockId) return null;
			const index = contextEngine.orderedBlockIds.indexOf(blockId);
			if (index === -1) return null;
			const set = /* @__PURE__ */ new Set();
			for (let i = 0; i <= index; i++) set.add(contextEngine.orderedBlockIds[i]);
			return set;
		});
		const derivedState = derived(() => {
			if (!entityId) return null;
			let eventIndex = -1;
			if (blockId) eventIndex = contextEngine.rawEvents.findIndex((e) => e.entity_id === entityId && e.block_id === blockId);
			let filteredEvents;
			if (eventIndex !== -1) filteredEvents = contextEngine.rawEvents.slice(0, eventIndex + 1);
			else {
				const filterSet = activeBlockIdsSetForNode();
				filteredEvents = contextEngine.rawEvents.filter((event) => {
					if (!filterSet) return true;
					if (!event.block_id) return event.event_type === "create" || event.event_type === "set_track";
					return filterSet.has(event.block_id);
				});
			}
			return reduceWikiEvents(filteredEvents, null, contextEngine.baseEntities).get(entityId) || null;
		});
		const finalMax = derived(() => derivedState()?.metadata?.max !== void 0 ? derivedState().metadata.max : max);
		const finalCurrent = derived(() => derivedState()?.metadata?.current !== void 0 ? derivedState().metadata.current : current);
		const finalName = derived(() => derivedState()?.name || name);
		const blockEvent = derived(() => blockId ? contextEngine.rawEvents.find((e) => e.block_id === blockId) : null);
		const textElement = derived(() => blockEvent()?.payload?.reason || blockEvent()?.payload?.description || blockEvent()?.payload?.content);
		$$renderer.push(`<div data-component="track-block-wrapper" class="group flex w-full items-center gap-4"><div data-component="track-inner-column" class="flex-1 flex flex-col gap-1"><div data-component="track-segments-row" class="flex h-4 w-full gap-1"><!--[-->`);
		const each_array = ensure_array_like(Array(finalMax()));
		for (let i = 0, $$length = each_array.length; i < $$length; i++) {
			each_array[i];
			$$renderer.push(`<div data-component="track-segment"${attr_class(`flex-1 rounded-sm border border-stone-700 transition-colors duration-300 ${stringify(i < finalCurrent() ? "bg-primary border-primary" : "bg-stone-900")}`)}></div>`);
		}
		$$renderer.push(`<!--]--></div> `);
		if (finalName()) {
			$$renderer.push("<!--[0-->");
			$$renderer.push(`<span data-component="track-name" class="text-[10px] font-bold tracking-widest text-stone-500 uppercase">${escape_html(finalName())}</span>`);
		} else $$renderer.push("<!--[-1-->");
		$$renderer.push(`<!--]--></div> `);
		if (textElement()) {
			$$renderer.push("<!--[0-->");
			$$renderer.push(`<div data-component="track-reason-container" class="flex flex-col justify-center border-l border-white/10 pl-4 max-w-[200px] shrink-0"><span data-component="track-reason-label" class="text-[9px] font-bold tracking-widest text-stone-500 uppercase">${escape_html(blockEvent()?.payload?.reason ? "Reason" : "Details")}</span> <p data-component="track-reason-text" class="mt-1 text-xs italic text-stone-400 leading-normal">"${escape_html(textElement())}"</p></div>`);
		} else $$renderer.push("<!--[-1-->");
		$$renderer.push(`<!--]--></div>`);
	});
}
//#endregion
//#region src/lib/editor/extensions/TrackBlock.ts
var TrackBlock = Node3.create({
	name: "trackBlock",
	group: "block",
	atom: true,
	addAttributes() {
	...
	parseHTML() {
		return [{ tag: "div[data-type=\"trackBlock\"]" }];
	},
	renderHTML({ HTMLAttributes }) {
	...
	renderText({ node }) {
		...
		const actionLabel = (node.attrs.action || "create") === "create" ? "Start" : "Progress";
		const blockEvent = node.attrs.id ? contextEngine.rawEvents.find((e) => e.block_id === node.attrs.id) : null;
		const reason = blockEvent?.payload?.reason || blockEvent?.payload?.description || blockEvent?.payload?.content;
		return `[TRACK: ${name} - ${actionLabel} (${current}/${max})${reason ? ` | "${reason}"` : ""}]`;
	},
	addNodeView() {
		return ({ node }) => {
			const dom = document.createElement("div");
			...
			mount(ProgressTrack, {
				...
				props: {
					entityId,
					blockId,
					max: node.attrs.max || 10,
					current: node.attrs.current || 0,
					name
				}
			});
			return {
...
//#endregion
//#region src/lib/components/mechanics/OracleResult.svelte
function OracleResult($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		$$renderer.push(`<div class="border-primary/20 bg-primary/5 group my-4 flex items-start gap-4 rounded-2xl border p-4 shadow-inner"><div class="bg-primary/20 text-primary rounded-xl p-2">`);
		if (type === "fate") {
			$$renderer.push("<!--[0-->");
			Circle_question_mark($$renderer, { size: 20 });
		} else {
			$$renderer.push("<!--[-1-->");
			Sparkles($$renderer, { size: 20 });
		}
		$$renderer.push(`<!--]--></div> <div class="flex-1 space-y-1">`);
		if (question) {
			$$renderer.push("<!--[0-->");
			$$renderer.push(`<div class="text-primary/70 mb-1 text-xs font-bold tracking-widest uppercase">${escape_html(type === "fate" ? "Fate Question" : "Theme Query")}</div> <div class="text-sm text-stone-400 italic">"${escape_html(question)}"</div>`);
		} else {
			$$renderer.push("<!--[-1-->");
			$$renderer.push(`<div class="text-primary/70 mb-1 text-xs font-bold tracking-widest uppercase">Oracle (${escape_html(type)})</div>`);
		}
		$$renderer.push(`<!--]--> <div${attr_class(`text-lg tracking-tight ${stringify(resultColor())}`)}>${escape_html(result)}</div></div></div>`);
	});
}
//#endregion
//#region src/lib/editor/extensions/OracleBlock.ts
var OracleBlock = Node3.create({
	name: "oracleBlock",
	group: "block",
	atom: true,
	addAttributes() {
	...
	parseHTML() {
		return [{ tag: "div[data-type=\"oracleBlock\"]" }];
	},
	renderHTML({ HTMLAttributes }) {
...
export { AllSelection as A, ClockBlock as C, DiceRoller as D, Extension as E, Fragment as F, GMNote as G, MappablePosition as M, Node as N, OddsCheck as O, Plugin as P, StatBlock as S, TrackBlock as T, OracleBlock as a, DOMSerializer as b, PluginKey as c, getChangedRanges as d, combineTransactionSteps as e, Slice as f, getSchema as g, TextSelection as h, index_default as i, NodeSelection as j, getUpdatedPosition as k, DecorationSet as l, Decoration as m, escapeForRegEx as n };
//# sourceMappingURL=index-8b3ef059.js.map
