import { a as attr_class, b as attr, c as escape_html, s as stringify } from './dev-db1ab9cf.js';
...
//#endregion
//#region src/routes/(author)/serials/[id]/scenes/[sceneId]/play/+page.svelte
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		let cursorState = { clocks: {} };
		let saveStatus = "synced";
		function handleHighlightBlock(blockId) {
		}
		function handleFocusBlock(blockId) {
		}
		let $$settled = true;
		let $$inner_renderer;
		function $$render_inner($$renderer) {
			$$renderer.push(`<div class="absolute inset-0 flex flex-col overflow-hidden bg-stone-950 font-sans text-stone-100"><div class="flex min-h-0 flex-1 overflow-hidden"><div class="animate-fade-in relative flex flex-1 flex-col items-center overflow-hidden px-8 pt-8"><div class="flex h-full min-h-0 w-full max-w-4xl flex-col"><div class="mb-6 flex shrink-0 items-center justify-between"><div class="flex items-center gap-2">`);
			$$renderer.push("<!--[0-->");
			$$renderer.push(`<div class="flex items-center gap-2 rounded-full border border-white/5 bg-white/5 px-3 py-1.5 text-[10px] font-bold tracking-widest text-stone-400 uppercase select-none transition-all duration-300 shadow-sm">`);
			...
			$$renderer.push(`<!--]--></div>`);
			$$renderer.push(`<!--]--></div> <div class="flex gap-3"><button${attr_class(`rounded-xl border p-2.5 shadow-sm transition-all ${stringify("border-white/10 bg-white/5 text-stone-400 hover:border-white/20 hover:text-white")}`)}${attr("title", "Reader Preview")}>`);
			Eye($$renderer, { class: "h-4 w-4" });
			$$renderer.push(`<!----></button> `);
			$$renderer.push("<!--[0-->");
			$$renderer.push(`<button${attr("disabled", editorComponent?.getIsSaving?.(), true)} class="bg-primary text-primary-foreground shadow-primary/20 flex items-center rounded-xl px-5 py-2 text-xs font-bold shadow-lg transition-all hover:opacity-90 disabled:opacity-50">`);
			Save($$renderer, { class: "mr-2 h-3.5 w-3.5" });
			$$renderer.push(`<!----> ${escape_html("Create Snapshot")}</button>`);
			$$renderer.push(`<!--]--></div></div> <div class="group relative min-h-0 flex-1">`);
			$$renderer.push("<!--[-1-->");
			Tiptap($$renderer, {
			...
			$$renderer.push(`<!--]--></div></div></div> <aside class="z-20 flex w-80 flex-col overflow-y-auto border-l border-white/5 bg-stone-900/20 shadow-2xl backdrop-blur-3xl"><div class="flex gap-2 border-b border-white/5 bg-white/5 p-4"><button${attr_class(`flex flex-1 items-center justify-center rounded-lg py-2 text-[10px] font-bold tracking-widest uppercase transition-all ${stringify("text-stone-500 hover:bg-white/5 hover:text-white")}`)}>`);
			Dices($$renderer, { class: "mr-2 h-3.5 w-3.5" });
			$$renderer.push(`<!----> Log</button> <button${attr_class(`flex flex-1 items-center justify-center rounded-lg py-2 text-[10px] font-bold tracking-widest uppercase transition-all ${stringify("bg-primary shadow-primary/20 text-white shadow-lg" )}`)}>`);
			...
			Activity($$renderer, { class: "mr-2 h-3.5 w-3.5" });
			$$renderer.push(`<!----> Mech</button></div> <div class="flex-1 overflow-hidden">`);
			{
				$$renderer.push("<!--[-1-->");
...
//# sourceMappingURL=_page.svelte-48acbf96.js.map
