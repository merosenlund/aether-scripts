...
export const BlockMetadata = Extension.create<BlockMetadataOptions>({
  ...
  addOptions() {
    return {
      types: ['paragraph', 'heading', 'blockquote', 'diceRoller', 'oddsCheck', 'gmNote', 'statBlock'],
    };
  },
    ...
    return [
      new Plugin({
        ...
        appendTransaction: (transactions, oldState, newState) => {
          const docChanges = transactions.some(transaction => transaction.docChanged) && !transactions.some(transaction => transaction.getMeta('blockMetadata'));
